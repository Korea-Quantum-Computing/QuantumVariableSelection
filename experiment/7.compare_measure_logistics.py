...
if __name__ == "__main__":
    ...
    for ite in range(args.number_of_test):
        ...
        thetasets = [[1 for i in range(args.number_features)],sa_mi_result,sa_full_result,sa_partial_result]
        ...
        for i in range(4) :
            ...
            R2_list += [bf.get_prediction_R2(dataset,y,0.9,y_type = y_type)]
    ...
    result_table = pd.DataFrame([QUBO_result,MSPE_result,R2_result,CN_result])
    result_table.columns = ["Original","Mutual Information","Simple R2","Partial R2"]
    result_table.index = ["QUBO_list","Accuracy","R2","CN"]


    random_index = str(random.randrange(0,99999)).zfill(5)
    
    result_table.to_csv("result/simulation_measure_logistics/linear_samples"+time+".csv")

    with open("result/simulation_measure_logistics/test_argument"+time+".txt", "w") as f:
